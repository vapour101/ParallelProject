...
\subsection{Worst Case Analysis}
 Pegs have to jump orthogannly so for any given state the maximum number of legal moves that can be made from that state is $4E=4(33-P)$, thus the game tree has a maximum branching factor of $4E$ at each level of the tree. As discussed above the number of moves to find a solution is $P-1$ but for some invalid paths the moves may lead to an invalid state that while $P>1$ no legal moves can be performed due to the relative positions of the remaining pegs. Thus $P-1$ is an upper bound on the depth with which any branch mis explored, i.e. the maximum height of the game tree is $P-1$.
